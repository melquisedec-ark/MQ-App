...
void main() {
  group('ChordParser - parseChordProLine', () {
    ...
    test('línea sin acordes retorna un solo segmento', () {
      ...
      expect(result, [const ChordLine(chord: null, text: text)]);
    });
    ...
    test('[C]Santo [G]Dios → 2 segmentos', () {
      ...
      expect(result[0], const ChordLine(chord: 'C', text: 'Santo '));
      expect(result[1], const ChordLine(chord: 'G', text: 'Dios'));
    });
    ...
    test('[C]Santo [G] → último segmento con texto vacío', () {
      ...
      expect(result.length, 2);
      expect(result[0], const ChordLine(chord: 'C', text: 'Santo '));
      expect(result[1], const ChordLine(chord: 'G', text: ''));
    });
    ...
    test('[Am][G][F] → múltiples acordes adyacentes', () {
      ...
      expect(result[0], const ChordLine(chord: 'Am', text: ''));
      expect(result[1], const ChordLine(chord: 'G', text: ''));
      expect(result[2], const ChordLine(chord: 'F', text: ''));
    });

    test('texto antes del primer acorde', () {
      final result = parseChordProLine('Intro [C]Santo');
      expect(result.length, 2);
      expect(result[0], const ChordLine(text: 'Intro '));
      expect(result[1], const ChordLine(chord: 'C', text: 'Santo'));
    });

    test('acorde con bajo G/B', () {
      final result = parseChordProLine('[G/B]Santo');
      expect(result.length, 1);
      expect(result[0], const ChordLine(chord: 'G/B', text: 'Santo'));
    });

    test('acorde menor Am', () {
      final result = parseChordProLine('[Am]Santo');
      expect(result.length, 1);
      expect(result[0], const ChordLine(chord: 'Am', text: 'Santo'));
    });

    test('acorde con séptima G7', () {
      final result = parseChordProLine('[G7]Santo');
      expect(result.length, 1);
      expect(result[0], const ChordLine(chord: 'G7', text: 'Santo'));
    });
  });
